...
// ─── StatBox with GSAP ────────────────────────────────────────────────────────
const StatBox = ({
  title, endValue, suffix, description, className
}: {
  title: string; endValue: number; suffix: string; description: string; className?: string;
}) => {
  ...
  useEffect(() => {
    if (reducedMotion) {
      if (numberRef.current) numberRef.current.innerText = endValue.toString() + suffix;
      return;
    }
      ...
      gsap.to(numberRef.current, {
        ...
        onUpdate: function() {
          if (numberRef.current) {
             numberRef.current.innerHTML = Math.round(Number(this.targets()[0].innerHTML)) + suffix;
          }
        }
      });
    }, boxRef);
...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tlHero.from(".h-script", { opacity: 0, duration: 1, ease: "power2.out" }, "-=0.4");
      tlHero.from(".h-body", { y: 20, opacity: 0, duration: 0.8, ease: "power4.out" }, "-=0.6");
      ...
      vmTl.from(".vm-card-vision", { x: -50, opacity: 0, duration: 0.8, ease: "power4.out" }, 0);
      vmTl.from(".vm-card-mission", { x: 50, opacity: 0, duration: 0.8, ease: "power4.out" }, 0);
      vmTl.from(".vm-card-vision li", { x: -15, opacity: 0, duration: 0.5, stagger: 0.06, ease: "power3.out" }, 0.4);
      vmTl.from(".vm-card-mission li", { x: 15, opacity: 0, duration: 0.5, stagger: 0.06, ease: "power3.out" }, 0.4);
    ...
    <div ref={containerRef} className="min-h-screen bg-brand-light flex flex-col">
      ...
      <section className="bg-brand-dark py-24 text-center relative overflow-hidden border-b-4 border-brand-orange">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold text-white tracking-tight uppercase mb-4 overflow-hidden pb-2">
            <span className="h-word inline-block" style={{ clipPath: "inset(100% 0% 0% 0%)" }}>Who&nbsp;</span>
            <span className="h-word inline-block" style={{ clipPath: "inset(100% 0% 0% 0%)" }}>We&nbsp;</span>
            <span className="h-word inline-block" style={{ clipPath: "inset(100% 0% 0% 0%)" }}>Are</span>
          </h1>
      ...
      <section className="stats-section bg-brand-dark border-b border-brand-gray shadow-2xl relative z-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 py-12">
            <StatBox className="stat-box" title="Certifications" endValue={5} suffix="+" description="To further reiterate our high Standards & Quality" />
            <StatBox className="stat-box" title="Project Success" endValue={100} suffix="%" description="Ensuring timely project completion & client satisfaction" />
            <StatBox className="stat-box" title="Employees" endValue={15} suffix="+" description="Qualified and skilled resources ensuring smooth operations" />
            <StatBox className="stat-box" title="Global Clients" endValue={100} suffix="+" description="Reaching a wider audience with excellent after-sales" />
          </div>
        </div>
      </section>
      ...
      <section className="profile-section py-24 max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16 team-element">
          ...
          <span className="underline-draw w-24 mx-auto" style={{ width: "6rem" }} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="profile-quote">
            <div className="quote-card bg-white p-10 md:p-14 rounded-sm shadow-xl border-l-8 border-brand-orange relative">
              <span className="absolute top-4 left-5 text-7xl text-gray-100 font-serif leading-none select-none">"</span>
            ...
            <div className="vm-card vm-card-mission bg-white p-10 shadow-lg border-t-4 border-brand-orange">
              ...
              <ul className="space-y-4 text-gray-600 font-medium">
                {[
                  ...
                  <li key={i} className="flex items-start gap-3">
                    <span className="text-brand-dark font-bold mt-0.5 flex-shrink-0">✓</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
      ...
      {/* ══════════ 6. CONTACT STRIP ══════════ */}
      <section className="contact-strip bg-brand-dark text-white py-16 border-t border-brand-gray">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/10">
          ...
          {[
            { icon: "📞", label: "Give us a call", content: <><p className="text-base font-medium">+91 20 26872522</p><p className="text-base font-medium">+91 9422308363</p></> },
            { icon: "✉️", label: "Email Us", content: <p className="text-base font-medium"><a href="mailto:sales@acs.co.in" className="hover:text-brand-orange transition-colors">sales@acs.co.in</a></p> },
            { icon: "📍", label: "India Works", content: <p className="text-sm text-white/50 leading-relaxed">Survey No. 52/3, Lane No. 4,<br />Maruti Nagar, Vadgaon Sheri,<br />Pune 411014.</p> }
          ].map((item, idx) => (
            <div key={idx} className="contact-col p-6 relative group">
              <div className="absolute left-0 top-0 w-[3px] h-0 bg-brand-orange transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:h-full" />
              <motion.span 
                className="text-3xl block mb-3 inline-block"
                whileHover={reducedMotion ? undefined : { scale: 1.1, rotate: 5 }}
                transition={{ type: "spring", stiffness: 300, damping: 15 }}
              >
                {item.icon}
              </motion.span>
              <h4 className="font-bold uppercase tracking-widest text-sm text-brand-orange mb-2">{item.label}</h4>
              {item.content}
            </div>
          ))}
